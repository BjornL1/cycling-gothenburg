...
Gothenburg cycling trips is offering bike tours in Gothenburg city (Sweden), the background for creating this site is to promote an environmentally friendly and easy way to explore Gothenburg city for tourists or anyone enjoying bicycling. Furthermore, the target group could be anyone newly moved to Gothenburg wishing to make new acquaintances and would suit well for team-activities.
...
Bicycling has becoming more popular recent years in Gothenburg, in combination with improving bike lanes, evolution of ebikes and a high ambition from local authorities to use enviromentally friendly transportation solutions, a new market oppurtunity has opened up. Gothenburg bug cycling trips can offer the increased demand for travel by bike.

-__Key goals for the site__
- To offer a bike tour through the city bu provding an easy accessibly website.
- To offer a platform for social events and flexibility to adapt for customer/users input by 
...
- __First time user:__
  ...
  - Navigating easyliy to acess the main functions.
...
One of the main colours used in the hero image (#252621) was re-used for the footer as well, the colour is still clearly marks a distinct difference compared to the contact section above it but still keeping the theme consistent with the landing page. The colour scheme was created with [coolors](https://coolors.co/). 
...
- Landing page
  - The header and navigation bar is adapted with media queries, once viewed on a smaller device the navbar will be placed under the header and both the header and navbar will be centered for promoting consistency in the display of the items and avoidng interference between them.

  - A link to the sign up form for form is displayed close to top center, the purpose with the element is to draw attention to the link since it is standing out from the page and leading directly to link to sign up form for the users.

- Bike tour content
  - This is where the main content is presented to the user, the section follows directly after the main page. The first image displayed is an overview of the Gothenburg city tour, the following content is illustrated by images from each tour section in the order following the route. For each image there is a short description of the different stops that are included in the tour. For smaller screens the content is displayed as a row the information text is followed by the image for each tour stop, by presenting the information in a row the image can be kept with a size making the image content clearly visible.
  ...
  - Fixed tour sign up form
    - In this form the user can submit a sign up form to join the fixed tour, the form also includes a possibility to rent different bikes. The name and email fields are required to fill in whereas the bike field is optional, however there is set quantity between 1 to 5 bikes preventing the user to not adding large quantities by mistakes. Furthermore there is a link to the rent a bike in the form field allowing the user to quikly check which bikes are avialibe before sending the form.
    The submit form button is intentionally increased in size to improve accessibility for small screen users.

- Rent a bike
  - In this section the bikes provided by Gothenburg cycling trips are presented, a table shows the cost for renting bike and below the table each of the bikes are illustrated with a heading. 
 On smaller screens the bike is changed to a row display instead in order to fit the content properly.

- Contact
  - The contact section is includling an address field but also phone and mail information, the latter is prepared with a link enabling the user to directly link to a mail improving in order to maintain accessibility.  

- A footer which contains social media icon links to facebook, twitter and youtube. Icons were used to keep the footer clean and because they are universally recognisable.


### Future Implementations
  - Use javascript to add a "to top" button with a solid smooth scroll functionality.  
...
- A "to the top" button with a smooth behaviour would have been preffered to include in the site, however sinceI do not have any experience so far with javascript and judging from best practice ti implement in a rational way java would be required this is something I am currently unable to implement, but would like to add to the website in a future update.

## Credits
### Code Used

- Specific coding
  - For handling header and nav elements to make a column and center the website  [adiati.com](https://adiati.com/how-to-create-a-responsive-navigation-bar-with-flexbox-and-media-queries) was an important contributor to solve the media qeury set up for smaller screens. In the section "Make The Navbar Responsive With Media Queries" partly the code shown was used in my website. The code can be found by searchning on the class ". header-container" in the css file.
